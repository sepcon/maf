...
#define MAF_CS_SIMPLE_REQUEST(RequestName, Input, Output)                      \
  struct RequestName##Request {                                                \
    ...
    template <typename... Args> static input_ptr make_input(Args &&... args) { \
...
struct TypeIDParamTrait {
  ...
  template <typename T> static constexpr bool IsRequest = true;
  ...
  translate(const CSPayloadIFPtr &csMsgContent,

 ...
 * that allows maf::srz::SerializationTrait<Json11::Json> and maf::srz::DumpHelper<json11::Json> to work
 * and allow using macros to generate classes that can read data from json/json_string with json11 is helper
 * to parse and convey the json data
 * To enable this support, you have to define macro `MAF_USING_JSON11` and include "json11.hpp" header before
 * include this file.
 * it'd better having some file like this:
 * HEADER: `MyJson11.h`
...
struct is_json11_convertible_type :
    public std::integral_constant<bool, std::is_same_v<std::string, T> || nstl::is_number_type_v<T> || is_tuple_like_v<T>>{};

template<typename T>
struct is_json11_convertible_type<std::vector<T>> :
    public std::integral_constant<bool, is_json11_convertible_type<typename std::vector<T>::value_type>::value>{};

template<typename Key, typename Value>
struct is_json11_convertible_type<std::map<Key, Value>> :
    public std::integral_constant< bool, std::is_same_v<std::string, Key> && is_json11_convertible_type<Value>::value >{};
...
struct J11TraitImpl
{
    ...
    {
        return internal::template get<Type>(j);
    }

    static bool exist(const Json& j)
    {
        return internal::template exist<Type>(j);
    }
...
{
    static json11::Json get(const Json& j)
    {
        return j;
    }
    static bool exist(const Json&)
    {
        return true;
    }
};
...
struct JsonTrait<json11::Json> : public JsonTraitBase<json11::Json>
{
    ...
    static size_t marshallSize(const Json& j)
    {
        return j.dump().size();
    }

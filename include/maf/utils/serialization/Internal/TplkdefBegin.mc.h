...
#define mc_maf_tuple_like_object_has_base_end_(ClassName) mc_maf_tuple_like_object_end(ClassName)


#define mc_maf_get_nth_option(_1, _2, option, ...) option
#define mc_maf_get_overload(...) mc_maf_msvc_expand_va_args( mc_maf_get_nth_option( __VA_ARGS__, mc_maf_tuple_like_object_has_base_, mc_maf_tuple_like_object_)(__VA_ARGS__) )
...
public: \
    using data_type = std::tuple< mc_remove_first_arg( mc_maf_for_each(mc_maf_take_only_first_arg, __VA_ARGS__) )>; \
    ...
    mc_maf_msvc_expand_va_args( mc_maf_define_load_from_json_functions(__VA_ARGS__) ) \
...
private: \
    ...
    data_type _data = data_type{ mc_remove_first_arg( mc_maf_for_each( mc_maf_get_default_value, __VA_ARGS__ ) ) }; \
    mc_maf_define_dump_helper_func


#define mc_maf_take_2_first_params_param_(first, second, ...) /*(*/ first, second /*)*/
#define mc_maf_take_2_first_params_param(...) mc_maf_msvc_expand_va_args ( mc_maf_take_2_first_params_param_( __VA_ARGS__ ) )

#define mc_maf_take_only_first_arg__(first, ...) /*(*/ , first /*)*/
#define mc_maf_take_only_first_arg_(...) mc_maf_msvc_expand_va_args( mc_maf_take_only_first_arg__(__VA_ARGS__) )
#define mc_maf_take_only_first_arg(firstsecond) mc_maf_take_only_first_arg_( mc_strip_parentheses(firstsecond) )

#define mc_maf_no_default(type, name)                   ,{}
#define mc_maf_has_default(type, name, TheDefaultValue)   ,TheDefaultValue
#define mc_maf_choose_correct_default_value_macro(_1, _2, candidate1, candidate, ...) candidate
#define mc_maf_get_default_value_(...) mc_maf_msvc_expand_va_args( mc_maf_choose_correct_default_value_macro(__VA_ARGS__, mc_maf_has_default, mc_maf_no_default)(__VA_ARGS__) )
#define mc_maf_get_default_value(SetOfParams) mc_maf_get_default_value_( mc_strip_parentheses(SetOfParams) )
...
#   define mc_maf_dump_each_property_with_index(TypeName, index) \
       mc_maf_dump_each_property( mc_maf_take_2_first_params_param( mc_strip_parentheses(TypeName) ), index)
...
#define mc_maf_declare_get_set_funcs_with_index(TypeName, index) \
    mc_maf_declare_get_set_func( mc_maf_take_2_first_params_param ( mc_strip_parentheses(TypeName) ), index)

#define mc_maf_declare_get_set_func(...) mc_maf_msvc_expand_va_args( mc_maf_declare_get_set_funcs_(__VA_ARGS__) )
...
#   define mc_maf_load_from_json_each_property(...) mc_maf_msvc_expand_va_args( mc_maf_load_json_on_each_property_(__VA_ARGS__) )

#   define mc_maf_load_json_on_each_property_with_index(TypeName, index) \
       mc_maf_load_from_json_each_property( mc_maf_take_2_first_params_param( mc_strip_parentheses(TypeName) ), index)
...
#   define mc_maf_define_load_from_json_functions(...) \
       ...
       void load_from_json(const JsonType& jsonIn) { \
           ...
           mc_maf_for_each_idx( mc_maf_load_json_on_each_property_with_index, __VA_ARGS__) \
       } \
       template<typename JsonType, std::enable_if_t<maf::srz::is_maf_compatible_json<JsonType>::value, bool> = true> \

...
ServiceProviderImpl::ServiceProviderImpl(ServiceID sid, ServiceProvider *holder,
                                         std::weak_ptr<ServerIF> server)
    : sid_{std::move(sid)}, server_(std::move(server)),
      delegator_(std::move(holder)) {
...
const ServiceID &ServiceProviderImpl::serviceID() const { return sid_; }
...
ActionCallStatus ServiceProviderImpl::broadcast(const OpID &propertyID,
                                                OpCode opCode,
...
CSPayloadIFPtr ServiceProviderImpl::getStatus(const OpID &propertyID) {
  std::lock_guard lock(propertyMap_);

...
static void serializationTest(OStream &theostream, IStream &theistream,
  ...
  // clang-format off
  MAF_TEST_CASE_BEGIN(stream_serialization_all_types)
  {
    MAF_TEST_EXPECT(ocomplex  == dcomplex )
    MAF_TEST_EXPECT(oi  == di )
    MAF_TEST_EXPECT(ol  == dl )
    MAF_TEST_EXPECT(of  == df )
    MAF_TEST_EXPECT(od  == dd )
    MAF_TEST_EXPECT(os  == ds )
    MAF_TEST_EXPECT(ovi  == dvi )
    MAF_TEST_EXPECT(oseti  == dseti )
    MAF_TEST_EXPECT(omsvi  == dmsvi )
    MAF_TEST_EXPECT(*osptr  == *dsptr )
    MAF_TEST_EXPECT(*orptr == *drptr)
    MAF_TEST_EXPECT(*ouptr == *duptr)
    MAF_TEST_EXPECT(oenum == denum)

    MAF_TEST_EXPECT( maf::srz::dump(ocomplex) == maf::srz::dump(dcomplex) )
    MAF_TEST_EXPECT( maf::srz::dump(oi) == maf::srz::dump(di) )
    MAF_TEST_EXPECT( maf::srz::dump(ol) == maf::srz::dump(dl) )
    MAF_TEST_EXPECT( maf::srz::dump(of) == maf::srz::dump(df) )
    MAF_TEST_EXPECT( maf::srz::dump(od) == maf::srz::dump(dd) )
    MAF_TEST_EXPECT( maf::srz::dump(os) == maf::srz::dump(ds) )
    MAF_TEST_EXPECT( maf::srz::dump(ovi) == maf::srz::dump(dvi) )
    MAF_TEST_EXPECT( maf::srz::dump(oseti) == maf::srz::dump(dseti) )
    MAF_TEST_EXPECT( maf::srz::dump(omsvi) == maf::srz::dump(dmsvi) )
    MAF_TEST_EXPECT( maf::srz::dump(*osptr) == maf::srz::dump(*dsptr) )
    MAF_TEST_EXPECT( maf::srz::dump(*orptr) == maf::srz::dump(*drptr) )
    MAF_TEST_EXPECT( maf::srz::dump(oenum) == maf::srz::dump(denum) )
  } MAF_TEST_CASE_END()
  // clang-format on
}
...
void dumpheperTest() {
  // clang-format off
  MAF_TEST_CASE_BEGIN(dump_helper_all_types)
  {
    MAF_TEST_EXPECT(maf::srz::dump(oi)  == "1000")
    MAF_TEST_EXPECT(maf::srz::dump(ol)  == "10000")
    MAF_TEST_EXPECT(maf::srz::dump(of)  == "200.000000")
    MAF_TEST_EXPECT(maf::srz::dump(od)  == "3000.000000")
    MAF_TEST_EXPECT(maf::srz::dump(os)  == R"("hello")")
    MAF_TEST_EXPECT(maf::srz::dump(ovi)  == "[\n  1,\n  2,\n  3\n]")
    MAF_TEST_EXPECT(maf::srz::dump(oseti)  == "[\n  1,\n  2,\n  3\n]")
    MAF_TEST_EXPECT(maf::srz::dump(omsvi)  == "{\n  \"hello\": [\n    1,\n    2,\n    3\n  ]\n}")
    MAF_TEST_EXPECT(maf::srz::dump(omsseti)  == "{\n  [\n    1,\n    2,\n    3\n  ]: [\n    1,\n    2,\n    3\n  ]\n}")
    MAF_TEST_EXPECT(maf::srz::dump(osptr)  == R"("Hello world")")
    MAF_TEST_EXPECT(maf::srz::dump(orptr)  == R"("hello world")")
    MAF_TEST_EXPECT(maf::srz::dump(ouptr)  == R"("hello world")")
    MAF_TEST_EXPECT(maf::srz::dump(oenum)  == "1")
  }
MAF_TEST_CASE_END(s)
  // clang-format on
}
